...
/**
 * Core Report class of objectives SVASU report plugin
...
namespace svasureport_objectives;

defined('MOODLE_INTERNAL') || die();

require_once($CFG->dirroot.'/mod/svasu/report/objectives/responsessettings_form.php');
...
class report extends \mod_svasu\report {
    ...
    public function display($svasu, $cm, $course, $download) {
        ...
        // Detailed report.
        $mform = new \mod_svasu_report_objectives_settings($PAGE->url, compact('currentgroup'));
        if ($fromform = $mform->get_data()) {
            $pagesize = $fromform->pagesize;
            $showobjectivescore = $fromform->objectivescore;
            set_user_preference('svasu_report_pagesize', $pagesize);
            set_user_preference('svasu_report_objectives_score', $showobjectivescore);
        } else {
            $pagesize = get_user_preferences('svasu_report_pagesize', 0);
            $showobjectivescore = get_user_preferences('svasu_report_objectives_score', 0);
        }
        if ($pagesize < 1) {
        ...
        if (empty($currentgroup)) {
            ...
            $groupstudents = get_users_by_capability($contextmodule, 'mod/svasu:savetrack',
        ...
        if ( !$nostudents ) {
            ...
            $objectives = get_svasu_objectives($svasu->id);
            $nosort = array();
            foreach ($objectives as $scoid => $sco) {
                foreach ($sco as $id => $objectivename) {
                    ...
                    if (!$displayoptions['objectivescore']) {
                        ...
                        $headers[] = $objectivename. ' '. get_string('status', 'svasureport_objectives');

                        // Now print objective score headers.
                        $colid = $scoid.'objectivescore' . $id;
                        $columns[] = $colid;
                        $nosort[] = $colid;
                        $headers[] = $objectivename. ' '. get_string('score', 'svasureport_objectives');
                    }
                }
            }
            ...
            if ($attempts) {
                foreach ($attempts as $scouser) {
                    ...
                    if (empty($timetracks->start)) {
                        ...
                        if (!$download) {
                            $url = new \moodle_url('/mod/svasu/report/userreport.php', array('id' => $cm->id,
 ...
 *
 * @param int $svasuid Scorm instance id
 * @return array an array of possible objectives.
 */
function get_svasu_objectives($svasuid) {
    global $DB;
    $objectives = array();
    $params = array();
    $select = "svasuid = ? AND ";
    ...
    $rs = $DB->get_recordset_select("svasu_scoes_track", $select, $params, 'value', "DISTINCT $value AS value, scoid");
    if ($rs->valid()) {
