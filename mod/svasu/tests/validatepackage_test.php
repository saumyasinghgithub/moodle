...
/**
 * Unit tests for {@link mod_svasu}.
 *
 * @copyright  2013 Dan Marsden
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_svasu_validatepackage_testcase extends advanced_testcase {
    ...
    public function test_validate_package() {
        ...
        $filename = "validsvasu.zip";
        $file = $this->create_stored_file_from_path($CFG->dirroot.'/mod/svasu/tests/packages/'.$filename, file_archive::OPEN);
        ...
        if (isset($errors['packagefile'])) {
            $this->assertEquals(get_string('nomanifest', 'svasu'), $errors['packagefile']);
        }

        $filename = "badsvasu.zip";
        $file = $this->create_stored_file_from_path($CFG->dirroot.'/mod/svasu/tests/packages/'.$filename, file_archive::OPEN);
        $errors = svasu_validate_package($file);
        $this->assertArrayHasKey('packagefile', $errors);
        if (isset($errors['packagefile'])) {
            $this->assertEquals(get_string('badimsmanifestlocation', 'svasu'), $errors['packagefile']);
        }
    }
}
