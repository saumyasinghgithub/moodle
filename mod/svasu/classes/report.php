...
// This class displays svasu reports. Because it is called from
// within /mod/svasu/report.php you can assume that the page header
...
class report {
    ...
    protected function generate_master_checkbox(): string {
        ...
        $selectalltext = get_string('selectall', 'svasu');
        $deselectalltext = get_string('selectnone', 'svasu');
        $mastercheckbox = new \core\output\checkbox_toggleall('svasu-attempts', true, [
            'name' => 'svasu-selectall-attempts',
            'value' => 1,
            'label' => $selectalltext,
    ...
    protected function generate_row_checkbox(string $name, string $value): string {
        ...
        $checkbox = new \core\output\checkbox_toggleall('svasu-attempts', false, [
            'name' => $name,
            'value' => $value,
        ]);
    ...
    protected function generate_delete_selected_button(): string {
        $deleteselectedparams = array(
            'type' => 'submit',
            'value' => get_string('deleteselected', 'svasu'),
            'class' => 'btn btn-secondary',
            'data-action' => 'toggle',
            'data-togglegroup' => 'svasu-attempts',
            'data-toggle' => 'action',
            'disabled' => true
        );
