...
class report extends \mod_svasu\report {
    ...
    protected function get_data($scoid, $allowedlist = []) {
        ...
        foreach ($attempts as $attempt) {
            if ($trackdata = svasu_get_tracks($scoid, $attempt->userid, $attempt->attempt)) {
    ...
    public function display($svasu, $cm, $course, $download) {
        ...
        if (empty($group)) {
            ...
            $students = get_users_by_capability($contextmodule, 'mod/svasu:savetrack', 'u.id' , '', '', '', '', '', false);
            $allowedlist = empty($students) ? array() : array_keys($students);
        } else {
            // All users who can attempt scoes and who are in the currently selected group.
            $groupstudents = get_users_by_capability($contextmodule, 'mod/svasu:savetrack', 'u.id', '', '', '', $group, '', false);
            $allowedlist = empty($groupstudents) ? array() : array_keys($groupstudents);
        }

        // Labels.
        $labels = [get_string('invaliddata', 'svasureport_graphs')];
        ...
        if ($scoes = $DB->get_records('svasu_scoes', array("svasu" => $svasu->id), 'sortorder, id')) {
            foreach ($scoes as $sco) {
                if ($sco->launch != '') {
                    ...
                    $chart->get_xaxis(0, true)->set_label(get_string('percent', 'svasureport_graphs'));
                    $yaxis = $chart->get_yaxis(0, true);
                    $yaxis->set_label(get_string('participants', 'svasureport_graphs'));
