 ...
 */
function svasu_report_list($context) {
    global $CFG;
    static $reportlist;
    if (!empty($reportlist)) {
        return $reportlist;
    }
    $installed = core_component::get_plugin_list('svasureport');
    foreach ($installed as $reportname => $notused) {

        // Moodle 2.8+ style of autoloaded classes.
        $classname = "svasureport_$reportname\\report";
 ...
 */
function get_svasu_question_count($svasuid) {
    global $DB;
    $count = 0;
    $params = array();
    $select = "svasuid = ? AND ";
    ...
    $rs = $DB->get_recordset_select("svasu_scoes_track", $select, $params, 'element');
