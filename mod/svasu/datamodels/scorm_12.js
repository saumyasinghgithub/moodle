...
//
// SVASU 1.2 API Implementation
//
function SVASUapi1_2(def, cmiobj, cmiint, cmistring256, cmistring4096, svasudebugging, svasuauto, svasuid, cfgwwwroot, sesskey,
    ...
    function LMSInitialize (param) {
        scoid = (svasu_current_node && svasu_current_node.scoid) ?  svasu_current_node.scoid : scoid;
        initdatamodel(scoid);
                ...
                if (typeof datamodel[scoid][elementmodel] != "undefined") {
                    if (datamodel[scoid][elementmodel].mod != 'r') {
                        ...
                        if (matches != null) {
                            ...
                            //Store data
                            if (errorCode == "0") {
                                if (autocommit && !(SVASUapi1_2.timeout)) {
                                    SVASUapi1_2.timeout = Y.later(60000, API, 'LMSCommit', [""], false);
                                }
    ...
    function LMSCommit (param) {
        if (SVASUapi1_2.timeout) {
            SVASUapi1_2.timeout.cancel();
            SVASUapi1_2.timeout = null;
        }
        errorCode = "0";
        if (param == "") {
            if (Initialized) {
    ...
    function StoreData(data,storetotaltime) {
        ...
        //alert('going to:' + "<?php p($CFG->wwwroot) ?>/mod/svasu/datamodel.php" + "id=<?php p($id) ?>&a=<?php p($a) ?>&sesskey=<?php echo sesskey() ?>"+datastring);
...
M.svasu_api.init = function(Y, def, cmiobj, cmiint, cmistring256, cmistring4096, svasudebugging, svasuauto, svasuid, cfgwwwroot,
        sesskey, scoid, attempt, viewmode, cmid, currentorg, autocommit, masteryoverride, hidetoc) {
    window.API = new SVASUapi1_2(def, cmiobj, cmiint, cmistring256, cmistring4096, svasudebugging, svasuauto, svasuid, cfgwwwroot,
            sesskey, scoid, attempt, viewmode, cmid, currentorg, autocommit, masteryoverride, hidetoc);
}
