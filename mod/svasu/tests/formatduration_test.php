...
/*
 * Unit tests for svasu_formatduration function from locallib.php
 *
 * @package    mod_svasu
 * @category   phpunit
...
global $CFG;
require_once($CFG->dirroot . '/mod/svasu/locallib.php'); // Include the code to test.


class mod_svasu_formatduration_testcase extends basic_testcase {
    public function test_svasu2004_format() {
        ...
        foreach ($suts as $key => $sut) {
            $formatted = svasu_format_duration($sut);
            $this->assertEquals($formatted, $validates[$key]);
        }
    }

    public function test_svasu12_format() {
        $stryears = get_string('years');
        $strmonths = trim(get_string('nummonths'));
        $strdays = get_string('days');
        $strhours = get_string('hours');
