...
function getLoggingActive () {
    var loggingActive = getCookie('SVASULoggingActive');
    if (!loggingActive) {
        loggingActive = 'A';
    }
...
function setLoggingActive (flag) {
    new cookie("SVASULoggingActive", flag, 365, "/").set();
}
...
function toggleLog () {
    if (getLoggingActive() == "A") {
        ...
        setLoggingActive('N');
        logButton.innerHTML = '-><?php echo addslashes_js(get_string('svasuloggingoff', 'svasu')); ?>';
    } else {
        setLoggingActive('A');
        AppendToLog("Moodle Logging Activated", 0);
        logButton.innerHTML = '<?php echo addslashes_js(get_string('svasuloggingon', 'svasu')); ?>';
...
var debugSVASUVersion = '<?php echo $svasu->version; ?>';
<?php
$lmsprefix = (svasu_version_check($svasu->version, SVASU_12) || empty($svasu->version)) ? 'LMS' : '';
$lmsapi = (svasu_version_check($svasu->version, SVASU_12) || empty($svasu->version)) ? 'API' : 'API_1484_11';

$lmselements = array();
if (svasu_version_check($svasu->version, SVASU_12) || empty($svasu->version)) {
...
function UpdateLog(s) {
    var s1 = '<html><head><style>\n'
        ...
        + '<\/style>'
        + '<script>\n'
        + 'var LMSVersion = \'<?php echo $svasu->version; ?>\';\n'
        + ' \n'
        ...
        + '          \' so API calls will fail - did navigate to another SVASU package?\'); \n'
        + '    return false; \n'
        + '} \n'
        + ' \n'
        + 'var saveElement = ""; \n'
        ...
        + '<h1>SVASU Debugging interface</h1>'
        + '<h2>SVASU Version Detected: <?php echo $svasu->version; ?></h2>'
        + '<input type="hidden" id="mod-svasu-logstate" name="mod-svasu-logstate" value="A" \/>'
...
?>
        ...
        + '             name="commitButton">'
        + '            <input type="button"'
        + '             value="<?php echo svasu_version_check($svasu->version, SVASU_12) ? 'LMSFinish' : 'Terminate'; ?>()  "'
        + '             onclick="try'
        + '             <?php echo svasu_version_check($svasu->version, SVASU_12) ? 'LMSFinish' : 'LMSTerminate'; ?>();"'
        + '             id="finishButton"'
        + '             name="finishButton">'
        + '         </td>'
        + '    </tr>'
        ...
        + '   </table>'
        + '</form>'
        + '<h3 id="mod-svasu-marker">SVASU API Activity Log<\/h3>';
    ...
    if (logPopUpWindow != 'N' && !logPopUpWindow.closed) {
        ...
        logPopUpWindow = open( '', 'svasulogpopupwindow', '' );
        popupdoc = logPopUpWindow.document;
        // Is logging active?
        var marker = safeGetElement(popupdoc, 'mod-svasu-marker');
        if (marker) {
            popupdoc.body.innerHTML += s;
        } else {
            popupdoc.open();
            ...
            popupdoc.close();
            popupdoc.title = 'SVASU API Activity Log';
            logPopUpWindow.focus();
        }
    }
...
function AppendToLog(s, rc) {
    ...
    s = '<div ' + sStyle + ' id="<?php echo $svasu->version; ?>">' + now.toGMTString() + ': ' + s + '<\/div>';
    UpdateLog(s);
    // switch colours for a new section of work
    if (s.match(/Commit|Loaded|Initialize|Terminate|Finish|Moodle SVASU|Moodle Logging/)) {
        logRow++;
    }
}
...
function LogAPICall(func, nam, val, rc) {
    ...
    s += ' => ' + String(rc) + "   scoid = " + svasu_current_node.scoid;
    AppendToLog(s, rc);
<?php
if (svasu_debugging($svasu) && ($sco->svasutype == 'asset')) {
?>
    hint = 'Item <?php echo $sco->identifier; ?> has been defined as an Asset: it should never call the SVASU API';
...
if (!document.getElementById('mod-svasu-log-toggle')) {
    var logButton = document.createElement('a');
    logButton.id = 'mod-svasu-log-toggle';
    ...
    var content = safeGetElement(document, 'svasupage');
