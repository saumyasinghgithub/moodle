...
/**
 * functions used by SVASU 1.2/2004 packages.
 *
 * @package    mod_svasu
...
function svasu_get_resources($blocks) {
    $resources = array();
...
function svasu_get_manifest($blocks, $scoes) {
    global $OUTPUT;
        ...
        foreach ($blocks as $block) {
            switch ($block['name']) {
                            ...
                            if ($metadata['name'] == 'SCHEMAVERSION') {
                                if (empty($scoes->version)) {
                                    ...
                                    if (isset($metadata['tagData']) && $isversionset) {
                                        $scoes->version = 'SVASU_'.$matches[count($matches) - 1];
                                    } else {
                                        $isversionset = (preg_match("/^2004 (3rd|4th) Edition$/", $metadata['tagData'], $matches));
                                        if (isset($metadata['tagData']) && $isversionset) {
                                            $scoes->version = 'SVASU_1.3';
                                        } else {
                                            $scoes->version = 'SVASU_1.2';
                                        }
                                    }
                                }
                            }
                ...
                case 'MANIFEST':
                    ...
                    $organization = '';
                    $resources = array();
                    $resources = svasu_get_resources($block['children']);
                    $scoes = svasu_get_manifest($block['children'], $scoes);
                    if (empty($scoes->elements) || count($scoes->elements) <= 0) {
                        foreach ($resources as $item => $resource) {
                            if (!empty($resource['HREF'])) {
                                ...
                                $sco->svasutype = $resource['ADLCP:SVASUTYPE'];
                ...
                case 'ORGANIZATION':
                    ...
                    $scoes->elements[$manifest][$organization][$identifier]->svasutype = '';
                ...
                case 'ITEM':
                    ...
                    if (!isset($block['attrs']['IDENTIFIERREF'])) {
                        $scoes->elements[$manifest][$organization][$identifier]->launch = '';
                        $scoes->elements[$manifest][$organization][$identifier]->svasutype = 'asset';
                    } else {
                        ...
                        if (!isset($resources[$idref])) {
                            ...
                            if (empty($resources[$idref]['ADLCP:SVASUTYPE'])) {
                                $resources[$idref]['ADLCP:SVASUTYPE'] = 'asset';
                            }
                            $scoes->elements[$manifest][$organization][$identifier]->svasutype = $resources[$idref]['ADLCP:SVASUTYPE'];
                        }
                    }
    ...
    if (!empty($manifestresourcesnotfound)) {
        ...
        echo $OUTPUT->notification(get_string('invalidmanifestresource', 'svasu').' '. implode(', ', $manifestresourcesnotfound));
    }
    return $scoes;
}

/**
 * Sets up SVASU 1.2/2004 packages using the manifest file.
 ...
 * @return bool
 */
function svasu_parse_svasu(&$svasu, $manifest) {
    global $CFG, $DB;
    ...
    $scoes = new stdClass();
    $scoes->version = '';
    $scoes = svasu_get_manifest($manifests, $scoes);
    $newscoes = array();
    $sortorder = 0;
    if (count($scoes->elements) > 0) {
        $olditems = $DB->get_records('svasu_scoes', array('svasu' => $svasu->id));
        foreach ($scoes->elements as $manifest => $organizations) {
            foreach ($organizations as $organization => $items) {
                foreach ($items as $identifier => $item) {
                    $sortorder++;
                    // This new db mngt will support all SVASU future extensions.
                    $newitem = new stdClass();
                    $newitem->svasu = $svasu->id;
                    $newitem->manifest = $manifest;
                    $newitem->organization = $organization;
                    $newitem->sortorder = $sortorder;
                    $standarddatas = array('parent', 'identifier', 'launch', 'svasutype', 'title');
                    ...
                    if (!empty($olditems) && ($olditemid = svasu_array_search('identifier', $newitem->identifier, $olditems))) {
                        $newitem->id = $olditemid;
                        // Update the Sco sortorder but keep id so that user tracks are kept against the same ids.
                        $DB->update_record('svasu_scoes', $newitem);
                        $id = $olditemid;
                        // Remove all old data so we don't duplicate it.
                        $DB->delete_records('svasu_scoes_data', array('scoid' => $olditemid));
                        $DB->delete_records('svasu_seq_objective', array('scoid' => $olditemid));
                        $DB->delete_records('svasu_seq_mapinfo', array('scoid' => $olditemid));
                        $DB->delete_records('svasu_seq_ruleconds', array('scoid' => $olditemid));
                        $DB->delete_records('svasu_seq_rulecond', array('scoid' => $olditemid));
                        $DB->delete_records('svasu_seq_rolluprule', array('scoid' => $olditemid));
                        $DB->delete_records('svasu_seq_rolluprulecond', array('scoid' => $olditemid));
                        ...
                        $id = $DB->insert_record('svasu_scoes', $newitem);
                    }
                    // Save this sco in memory so we can use it later.
                    $newscoes[$id] = $newitem;

                    if ($optionaldatas = svasu_optionals_data($item, $standarddatas)) {
                        $data = new stdClass();
                        $data->scoid = $id;
                        foreach ($optionaldatas as $optionaldata) {
                            if (isset($item->$optionaldata)) {
                                ...
                                $dataid = $DB->insert_record('svasu_scoes_data', $data);
                            }
                        }
                    }
                    ...
                    if (isset($item->sequencingrules)) {
                        foreach ($item->sequencingrules as $sequencingrule) {
                            ...
                            $ruleid = $DB->insert_record('svasu_seq_ruleconds', $rule);
                            if (isset($sequencingrule->ruleconditions)) {
                                foreach ($sequencingrule->ruleconditions as $rulecondition) {
                                    ...
                                    $rulecondid = $DB->insert_record('svasu_seq_rulecond', $rulecond);
                                }
                            }
                        }
                    }
                    ...
                    if (isset($item->rolluprules)) {
                        foreach ($item->rolluprules as $rolluprule) {
                            ...
                            $rollupruleid = $DB->insert_record('svasu_seq_rolluprule', $rollup);
                            if (isset($rollup->conditions)) {
                                foreach ($rollup->conditions as $condition) {
                                    ...
                                    $conditionid = $DB->insert_record('svasu_seq_rolluprulecond', $cond);
                                }
                            }
                        }
                    }
                    ...
                    if (isset($item->objectives)) {
                        foreach ($item->objectives as $objective) {
                            ...
                            $objectiveid = $DB->insert_record('svasu_seq_objective', $obj);
                            if (isset($objective->mapinfos)) {
                                foreach ($objective->mapinfos as $objmapinfo) {
                                    ...
                                    $mapinfoid = $DB->insert_record('svasu_seq_mapinfo', $mapinfo);
                                }
                            }
                        }
                    }
        ...
        if (!empty($olditems)) {
            foreach ($olditems as $olditem) {
                $DB->delete_records('svasu_scoes', array('id' => $olditem->id));
                $DB->delete_records('svasu_scoes_data', array('scoid' => $olditem->id));
                $DB->delete_records('svasu_scoes_track', array('scoid' => $olditem->id));
                $DB->delete_records('svasu_seq_objective', array('scoid' => $olditem->id));
                $DB->delete_records('svasu_seq_mapinfo', array('scoid' => $olditem->id));
                $DB->delete_records('svasu_seq_ruleconds', array('scoid' => $olditem->id));
                $DB->delete_records('svasu_seq_rulecond', array('scoid' => $olditem->id));
                $DB->delete_records('svasu_seq_rolluprule', array('scoid' => $olditem->id));
                $DB->delete_records('svasu_seq_rolluprulecond', array('scoid' => $olditem->id));
            }
        }
        if (empty($scoes->version)) {
            $scoes->version = 'SVASU_1.2';
        }
        $DB->set_field('svasu', 'version', $scoes->version, array('id' => $svasu->id));
        $svasu->version = $scoes->version;
    }
    $svasu->launch = 0;
    ...
    if (!empty($defaultorgid) && isset($newscoes[$defaultorgid]) && !empty($newscoes[$defaultorgid]->launch)) {
        ...
        $sqlselect = 'svasu = ? AND sortorder >= ? AND '.$DB->sql_isnotempty('svasu_scoes', 'launch', false, true);
        // We use get_records here as we need to pass a limit in the query that works cross db.
        $scoes = $DB->get_records_select('svasu_scoes', $sqlselect, array($svasu->id, $firstinorg), 'sortorder', 'id', 0, 1);
        if (!empty($scoes)) {
    ...
    if (empty($svasu->launch)) {
        ...
        // We use get_records here as we need to pass a limit in the query that works cross db.
        $scoes = $DB->get_records_select('svasu_scoes', $sqlselect, array($svasu->id), 'sortorder', 'id', 0, 1);
        if (!empty($scoes)) {
            $sco = reset($scoes); // We only care about the first record - the above query only returns one.
            $svasu->launch = $sco->id;
        }
...
function svasu_optionals_data($item, $standarddata) {
    $result = array();
...
function svasu_is_leaf($sco) {
    global $DB;

    if ($DB->record_exists('svasu_scoes', array('svasu' => $sco->svasu, 'parent' => $sco->identifier))) {
        return false;
    }
    return true;
}

function svasu_get_parent($sco) {
    global $DB;

    if ($sco->parent != '/') {
        if ($parent = $DB->get_record('svasu_scoes', array('svasu' => $sco->svasu, 'identifier' => $sco->parent))) {
            return svasu_get_sco($parent->id);
        }
    }
    return null;
}

function svasu_get_children($sco) {
    global $DB;

    $children = $DB->get_records('svasu_scoes', array('svasu' => $sco->svasu, 'parent' => $sco->identifier), 'sortorder, id');
    if (!empty($children)) {
        return $children;
    }
    return null;
...
function svasu_get_available_children($sco) {
    global $DB;

    $res = $DB->get_records('svasu_scoes', array('svasu' => $sco->svasu, 'parent' => $sco->identifier), 'sortorder, id');
    if (!$res || $res == null) {
        return false;
    } else {
        foreach ($res as $sco) {
...
function svasu_get_available_descendent($descend = array(), $sco) {
    if ($sco == null) {
        return $descend;
    } else {
        $avchildren = svasu_get_available_children($sco);
        foreach ($avchildren as $avchild) {
            array_push($descend, $avchild);
        }
        foreach ($avchildren as $avchild) {
            svasu_get_available_descendent($descend, $avchild);
        }
    }
}

function svasu_get_siblings($sco) {
    global $DB;

    if ($siblings = $DB->get_records('svasu_scoes', array('svasu' => $sco->svasu, 'parent' => $sco->parent), 'sortorder, id')) {
...
function svasu_get_ancestors($sco) {
    $ancestors = array();
    $continue = true;
    while ($continue) {
        $ancestor = svasu_get_parent($sco);
...
function svasu_get_preorder(&$preorder = array(), $sco = null) {
    if ($sco != null) {
        array_push($preorder, $sco);
        if ($children = svasu_get_children($sco)) {
            foreach ($children as $child) {
                svasu_get_preorder($preorder, $child);
            }
        }
    }
    return $preorder;
...
function svasu_find_common_ancestor($ancestors, $sco) {
    $pos = svasu_array_search('identifier', $sco->parent, $ancestors);
    if ($sco->parent != '/') {
        if ($pos === false) {
            return svasu_find_common_ancestor($ancestors, svasu_get_parent($sco));
        }
    }
    return $pos;
}
