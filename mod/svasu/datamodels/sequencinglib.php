...
function svasu_seq_exit_action_rules($seq, $userid) {
    $sco = $seq->currentactivity;
    $ancestors = svasu_get_ancestors($sco);
    $exittarget = null;
    foreach (array_reverse($ancestors) as $ancestor) {
        if (svasu_seq_rules_check($ancestor, 'exit') != null) {
            $exittarget = $ancestor;
            break;
        }
    }
    if ($exittarget != null) {
        $commons = array_slice($ancestors, 0, svasu_find_common_ancestor($ancestors, $exittarget));

        // Terminate Descendent Attempts Process.
        if ($commons) {
            foreach ($commons as $ancestor) {
                svasu_seq_end_attempt($ancestor, $userid, $seq->attempt);
...
function svasu_seq_post_cond_rules($seq, $userid) {
    $sco = $seq->currentactivity;
    if (!$seq->suspended) {
        if ($action = svasu_seq_rules_check($sco, 'post') != null) {
            switch($action) {
                case 'retry':
                case 'continue':
                case 'previous':
...
function svasu_seq_evaluate_rollupcond($sco, $conditioncombination, $rollupruleconds, $userid) {
    $bag = Array();
    $con = "";
    $val = false;
    $unk = false;
    foreach ($rollupruleconds as $rolluprulecond) {
        $condit = svasu_evaluate_condition($rolluprulecond, $sco, $userid);
...
function svasu_seq_check_child ($sco, $action, $userid) {
    global $DB;

    $included = false;
    $sco = svasu_get_sco($sco->id);
    $r = $DB->get_record('svasu_scoes_track', array('scoid' => $sco->id, 'userid' => $userid, 'element' => 'activityattemptcount'));
    if ($action == 'satisfied' || $action == 'notsatisfied') {
        if (!$sco->rollupobjectivesatisfied) {
            ...
            if (($action == 'satisfied' && $sco->requiredforsatisfied == 'ifnotsuspended') ||
                ...
                if (!svasu_seq_is('activityprogressstatus', $sco->id, $userid) ||
                    ((($r->value) > 0) && !svasu_seq_is('suspended', $sco->id, $userid))) {
                    $included = false;
                }
...
function svasu_seq_sequencing ($scoid, $userid, $seq) {

    switch ($seq->sequencing) {
        case 'start':
            // We'll see the parameters we have to send, this should update delivery and end.
            $seq = svasu_seq_start_sequencing($scoid, $userid, $seq);
            $seq->sequencing = true;
            break;

        case 'resumeall':
            // We'll see the parameters we have to send, this should update delivery and end.
            $seq = svasu_seq_resume_all_sequencing($scoid, $userid, $seq);
            $seq->sequencing = true;
            break;

        case 'exit':
            // We'll see the parameters we have to send, this should update delivery and end.
            $seq = svasu_seq_exit_sequencing($scoid, $userid, $seq);
            $seq->sequencing = true;
            break;

        case 'retry':
            // We'll see the parameters we have to send, this should update delivery and end.
            $seq = svasu_seq_retry_sequencing($scoid, $userid, $seq);
            $seq->sequencing = true;
            break;

        case 'previous':
            // We'll see the parameters we have to send, this should update delivery and end.
            $seq = svasu_seq_previous_sequencing($scoid, $userid, $seq);
            $seq->sequencing = true;
            break;

        case 'choice':
            // We'll see the parameters we have to send, this should update delivery and end.
            $seq = svasu_seq_choice_sequencing($scoid, $userid, $seq);
            $seq->sequencing = true;
            break;
    }
...
function svasu_seq_start_sequencing($scoid, $userid, $seq) {
    global $DB;
    ...
    $sco = $DB->get_record('svasu_scoes', array('scoid' => $scoid, 'userid' => $userid));
    if (($sco->parent == '/') && svasu_is_leaf($sco)) { // If the activity is the root and is leaf.
        $seq->delivery = $sco;
    } else {
        $ancestors = svasu_get_ancestors($sco);
        $ancestorsroot = array_reverse($ancestors);
        $res = svasu_seq_flow($ancestorsroot[0], 'forward', $seq, true, $userid);
        if ($res) {
            return $res;
        }
    }
...
function svasu_seq_resume_all_sequencing($scoid, $userid, $seq) {
    global $DB;
    ...
    $track = $DB->get_record('svasu_scoes_track', array('scoid' => $scoid, 'userid' => $userid, 'element' => 'suspendedactivity'));
    if (!$track) {
        $seq->delivery = null;
    ...
    $seq->delivery = $DB->get_record('svasu_scoes', array('scoid' => $scoid, 'userid' => $userid));
}

function svasu_seq_continue_sequencing($scoid, $userid, $seq) {
    if (empty($seq->currentactivity)) {
        $seq->delivery = null;
    ...
    if ($currentact->parent != '/') {
        ...
        $parent = svasu_get_parent ($currentact);
        ...
        $res = svasu_seq_flow($currentact, 'forward', $seq, false, $userid);
        if ($res) {
            return $res;
        }
    }
...
function svasu_seq_previous_sequencing($scoid, $userid, $seq) {
    if (empty($seq->currentactivity)) {
        $seq->delivery = null;
    ...
    if ($currentact->parent != '/') { // If the activity is the root and is leaf.
        ...
        $res = svasu_seq_flow($currentact, 'backward', $seq, false, $userid);
        if ($res) {
            return $res;
        }
    }
...
function svasu_seq_exit_sequencing($scoid, $userid, $seq) {
    if (empty($seq->currentactivity)) {
        $seq->delivery = null;
...
function svasu_seq_retry_sequencing($scoid, $userid, $seq) {
    if (empty($seq->currentactivity)) {
        $seq->delivery = null;
    ...
    if (!svasu_is_leaf($seq->currentactivity)) {
        $res = svasu_seq_flow($seq->currentactivity, 'forward', $seq, true, $userid);
        if ($res != null) {
            return $res;
        } else {
            // Return deliver.
...
function svasu_seq_choice_sequencing($sco, $userid, $seq) {
    ...
    foreach ($arrpath as $activity) {
        if ($activity->parent != '/') {
            $avchildren = svasu_get_available_children (svasu_get_parent($activity));
        ...
        if (svasu_seq_rules_check($activity, 'hidefromchoice' != null)) {
            $seq->delivery = null;
    ...
    if ($sco->parent != '/') {
        $parent = svasu_sco_get_parent ($sco);
    ...
    if ($seq->currentactivity != null) {
        $commonpos = svasu_find_common_ancestor($ancestors, $seq->currentactivity);
    ...
    if ($seq->currentactivity === $sco) {
        // MDL-51757 - this part of the SVASU 2004 sequencing and navigation was not completed.
        throw new \coding_exception('Unexpected state encountered');
    }

    $sib = svasu_get_siblings($seq->currentactivity);
    ...
    if ($pos !== false) {
        ...
        $children = svasu_get_children (svasu_get_parent ($sco));
        ...
        foreach ($siblings as $sibling) {
            $seq = svasu_seq_choice_activity_traversal($sibling, $userid, $seq, $traverse);
            if (!$seq->reachable) {
                $seq->delivery = null;
                return $seq;
            }
    ...
    if ($seq->currentactivity == null || $seq->currentactivity == $comancestor) {
        ...
        foreach ($comtarget as $act) {
            $seq = svasu_seq_choice_activity_traversal($act, $userid, $seq, 'forward');
            if (!$seq->reachable) {
                $seq->delivery = null;
                return $seq;
            }
            $act = svasu_get_sco ($acti->id);
            if (svasu_seq_is('active', $act->id, $userid) && ($act->id != $comancestor->id && $act->preventactivation)) {
                $seq->delivery = null;
    ...
    if ($comancestor->id == $sco->id) {
        ...
        $ancestorscurrent = svasu_get_ancestors($seq->currentactivity);
    ...
    if (array_search($ancestors, $comancestor) !== false) {
        $ancestorscurrent = svasu_get_ancestors($seq->currentactivity);
        $commonpos = svasu_find_common_ancestor($ancestors, $sco);
        ...
        if ($constrained != null) {
            $fwdir = svasu_get_preorder($constrained);
            ...
            $seq = svasu_seq_choice_flow($constrained, $traverse, $seq);
            $actconsider = $seq->identifiedactivity;
            $avdescendents = Array();
            $avdescendents = svasu_get_available_descendents($actconsider);
        ...
        $fwdir = svasu_get_preorder($seq->currentactivity);
        ...
        if (array_search($fwdir, $sco) !== false) {
            foreach ($comtarget as $act) {
                ...
                $act = svasu_get_sco($act->id);
                if (svasu_seq_is('active', $act->id, $userid) && ($act->id != $comancestor->id &&
            ...
            foreach ($comtarget as $act) {
                $act = svasu_get_sco($act->id);
                if (svasu_seq_is('active', $act->id, $userid) && $act->id != $comancestor->id && $act->preventactivation == true) {
                    $seq->delivery = null;
                    $seq->exception = 'SB.2.9-6';
                    return $seq;
                }
    ...
    $seq = svasu_seq_flow($sco, 'forward', $seq, true, $userid);
    if ($seq->deliverable == false) {
        svasu_terminate_descendent_attempts($comancestor, $userid, $seq);
        svasu_seq_end_attempt($comancestor, $userid, $seq->attempt);
...
function svasu_seq_choice_flow ($constrained, $traverse, $seq) {
    $seq = svasu_seq_choice_flow_tree ($constrained, $traverse, $seq);
...
function svasu_seq_choice_flow_tree ($constrained, $traverse, $seq) {
    $islast = false;
    $parent = svasu_get_parent ($constrained);
    if ($traverse == 'forward') {
        $preord = svasu_get_preorder($constrained);
        ...
        $avchildren = svasu_get_available_children($parent); // Available children.
        if ($avchildren[count($avchildren) - 1]->id == $constrained->id) {
            $seq = svasu_seq_choice_flow_tree ($parent, 'forward', $seq);
            return $seq;
        } else {
            $i = 0;
    ...
    if ($traverse == 'backward') {
        ...
        if ($avchildren [0]->id == $constrained->id) {
            $seq = svasu_seq_choice_flow_tree ($parent, 'backward', $seq);
            return $seq;
        } else {
...
function svasu_seq_choice_activity_traversal($activity, $userid, $seq, $direction) {
    if ($direction == 'forward') {
        $act = svasu_seq_rules_check($activity, 'stopforwardtraversal');
    ...
    if ($direction == 'backward') {
        $parentsco = svasu_get_parent($activity);
        if ($parentsco != null) {
...
function svasu_sequencing_delivery($scoid, $userid, $seq) {

    if (!svasu_is_leaf($seq->delivery)) {
        $seq->deliveryvalid = false;
        $seq->exception = 'DB.1.1-1';
        return $seq;
    }
    $ancestors = svasu_get_ancestors($seq->delivery);
    $arrpath = array_reverse($ancestors);
    ...
    foreach ($arrpath as $activity) {
        if (svasu_check_activity($activity, $userid)) {
            $seq->deliveryvalid = false;
...
function svasu_content_delivery_environment($seq, $userid) {
    global $DB;

    $act = $seq->currentactivity;
    if (svasu_seq_is('active', $act->id, $userid)) {
        $seq->exception = 'DB.2-1';
        return $seq;
    }
    $track = $DB->get_record('svasu_scoes_track', array('scoid' => $act->id,
                                                        'userid' => $userid,
                                                        'element' => 'suspendedactivity'));
    if ($track != null) {
        $seq = svasu_clear_suspended_activity($seq->delivery, $seq, $userid);

    }
    $seq = svasu_terminate_descendent_attempts ($seq->delivery, $userid, $seq);
    $ancestors = svasu_get_ancestors($seq->delivery);
    $arrpath = array_reverse($ancestors);
    ...
    foreach ($arrpath as $activity) {
        if (!svasu_seq_is('active', $activity->id, $userid)) {
            if (!isset($activity->tracked) || ($activity->tracked == 1)) {
                if (!svasu_seq_is('suspended', $activity->id, $userid)) {
                    $r = $DB->get_record('svasu_scoes_track', array('scoid' => $activity->id,
                                                                    'userid' => $userid,
                                                                    'element' => 'activityattemptcount'));
                    $r->value = ($r->value) + 1;
                    $DB->update_record('svasu_scoes_track', $r);
                    if ($r->value == 1) {
                        svasu_seq_set('activityprogressstatus', $activity->id, $userid, 'true');
                    }
                    svasu_insert_track($userid, $activity->svasu, $activity->id, 0, 'objectiveprogressstatus', 'false');
                    svasu_insert_track($userid, $activity->svasu, $activity->id, 0, 'objectivesatisfiedstatus', 'false');
                    svasu_insert_track($userid, $activity->svasu, $activity->id, 0, 'objectivemeasurestatus', 'false');
                    svasu_insert_track($userid, $activity->svasu, $activity->id, 0, 'objectivenormalizedmeasure', 0.0);

                    svasu_insert_track($userid, $activity->svasu, $activity->id, 0, 'attemptprogressstatus', 'false');
                    svasu_insert_track($userid, $activity->svasu, $activity->id, 0, 'attemptcompletionstatus', 'false');
                    svasu_insert_track($userid, $activity->svasu, $activity->id, 0, 'attemptabsoluteduration', 0.0);
                    svasu_insert_track($userid, $activity->svasu, $activity->id, 0, 'attemptexperiencedduration', 0.0);
                    svasu_insert_track($userid, $activity->svasu, $activity->id, 0, 'attemptcompletionamount', 0.0);
                }
            }
            svasu_seq_set('active', $activity->id, $userid, 'true');
        }
    }
    $seq->delivery = $seq->currentactivity;
    svasu_seq_set('suspendedactivity', $activity->id, $userid, 'false');
    ...
    if (isset($activity->tracked) || ($activity->tracked == 0)) {
        ...
        $atabsdur = $DB->get_record('svasu_scoes_track', array('scoid' => $activity->id,
                                                                'userid' => $userid,
                                                                'element' => 'attemptabsoluteduration'));
        $atexpdur = $DB->get_record('svasu_scoes_track', array('scoid' => $activity->id,
                                                                'userid' => $userid,
...
function svasu_clear_suspended_activity($act, $seq, $userid) {
    global $DB;
    $currentact = $seq->currentactivity;
    $track = $DB->get_record('svasu_scoes_track', array('scoid' => $currentact->id,
                                                        'userid' => $userid,
                                                        'element' => 'suspendedactivity'));
    if ($track != null) {
        $ancestors = svasu_get_ancestors($act);
        $commonpos = svasu_find_common_ancestor($ancestors, $currentact);
        if ($commonpos !== false) {
                ...
                if (!empty($activitypath)) {
                    ...
                    foreach ($activitypath as $activity) {
                        if (svasu_is_leaf($activity)) {
                            svasu_seq_set('suspended', $activity->id, $userid, false);
                        } else {
                            $children = svasu_get_children($activity);
                            $bool = false;
                            foreach ($children as $child) {
                                if (svasu_seq_is('suspended', $child->id, $userid)) {
                                    $bool = true;
                                }
                            }
                            if (!$bool) {
        ...
        svasu_seq_set('suspendedactivity', $act->id, $userid, false);
    }
}

function svasu_select_children_process($scoid, $userid) {
    global $DB;

    $sco = svasu_get_sco($scoid);
    if (!svasu_is_leaf($sco)) {
        if (!svasu_seq_is('suspended', $scoid, $userid) && !svasu_seq_is('active', $scoid, $userid)) {
            $r = $DB->get_record('svasu_scoes_track', array('scoid' => $scoid,
                                                            'userid' => $userid,
            ...
            switch ($r->value) {
                ...
                case 'once':
                    if (!svasu_seq_is('activityprogressstatus', $scoid, $userid)) {
                        if (svasu_seq_is('selectioncountsstatus', $scoid, $userid)) {
                            $childlist = '';
                            $res = $DB->get_record('svasu_scoes_track', array('scoid' => $scoid,
                                                                                'userid' => $userid,
                            ...
                            svasu_seq_set('availablechildren', $scoid, $userid, false);
                            svasu_seq_set('availablechildren', $scoid, $userid, $clist);
                        }
                    }
                break;
            }
...
function svasu_randomize_children_process($scoid, $userid) {
    global $DB;
    ...
    if (!svasu_is_leaf($sco)) {
        if (!svasu_seq_is('suspended', $scoid, $userid) && !svasu_seq_is('active', $scoid, $userid)) {
            ...
            switch ($r->value) {
                ...
                case 'once':
                    if (!svasu_seq_is('activityprogressstatus', $scoid, $userid)) {
                        if (svasu_seq_is('randomizechildren', $scoid, $userid)) {
                            $childlist = array();
                            $res = svasu_get_available_children($sco);
                            $i = count($res) - 1;
                            $children = $res->value;
...
function svasu_terminate_descendent_attempts($activity, $userid, $seq) {
    $ancestors = svasu_get_ancestors($seq->currentactivity);
    $commonpos = svasu_find_common_ancestor($ancestors, $activity);
    if ($commonpos !== false) {
        if ($activitypath = array_slice($ancestors, 1, $commonpos - 2)) {
            if (!empty($activitypath)) {
                foreach ($activitypath as $sco) {
                    svasu_seq_end_attempt($sco, $userid, $seq->attempt);
                }
            }
        }
    }
...
function svasu_sequencing_exception($seq) {
    global $OUTPUT;
    if ($seq->exception != null) {
