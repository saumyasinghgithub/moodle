 ...
 * @return mixed
 */
function svasu_get_aicc_columns($row, $mastername='system_id') {
 ...
 * @return string
 */
function svasu_forge_cols_regexp($columns, $remodule='(".*")?,') {
    $regexp = '/^';
 ...
 * @return bool
 */
function svasu_parse_aicc(&$svasu) {
    global $DB;

    if ($svasu->svasutype == SVASU_TYPE_AICCURL) {
        return svasu_aicc_generate_simple_sco($svasu);
    }
    if (!isset($svasu->cmid)) {
    ...
    $files = $fs->get_area_files($context->id, 'mod_svasu', 'content', 0, 'sortorder, itemid, filepath, filename', false);
    ...
    foreach ($ids as $courseid => $id) {
        ...
        if (isset($id->des)) {
            ...
            $rows = explode("\r\n", $contents);
            $columns = svasu_get_aicc_columns($rows[0]);
            $regexp = svasu_forge_cols_regexp($columns->columns);
        ...
        if (isset($id->cst)) {
            ...
            $rows = explode("\r\n", $contents);
            $columns = svasu_get_aicc_columns($rows[0], 'block');
            $regexp = svasu_forge_cols_regexp($columns->columns, '(.+)?,');
            for ($i = 1; $i < count($rows); $i++) {
                if (preg_match($regexp, $rows[$i], $matches)) {
                    for ($j = 0; $j < count($columns->columns); $j++) {
        ...
        if (isset($id->ort)) {
            ...
            $rows = explode("\r\n", $contents);
            $columns = svasu_get_aicc_columns($rows[0], 'course_element');
            $regexp = svasu_forge_cols_regexp($columns->columns, '(.+)?,');
            for ($i = 1; $i < count($rows); $i++) {
                if (preg_match($regexp, $rows[$i], $matches)) {
        ...
        if (isset($id->pre)) {
            ...
            $rows = explode("\r\n", $contents);
            $columns = svasu_get_aicc_columns($rows[0], 'structure_element');
            $regexp = svasu_forge_cols_regexp($columns->columns, '(.+),');
            for ($i = 1; $i < count($rows); $i++) {
                if (preg_match($regexp, $rows[$i], $matches)) {
    ...
    $oldscoes = $DB->get_records('svasu_scoes', array('svasu' => $svasu->id));
    $sortorder = 0;
    $launch = 0;
    if (isset($courses)) {
        foreach ($courses as $course) {
            ...
            if ($ss = $DB->get_record('svasu_scoes', array('svasu' => $svasu->id,
                                                           'identifier' => $sco->identifier))) {
                $id = $ss->id;
                $sco->id = $id;
                $DB->update_record('svasu_scoes', $sco);
                unset($oldscoes[$id]);
            } else {
                $id = $DB->insert_record('svasu_scoes', $sco);
            }
            ...
            if (isset($course->elements)) {
                foreach ($course->elements as $element) {
                    ...
                    if ($oldscoid = svasu_array_search('identifier', $sco->identifier, $oldscoes)) {
                        $sco->id = $oldscoid;
                        $DB->update_record('svasu_scoes', $sco);
                        $id = $oldscoid;
                        $DB->delete_records('svasu_scoes_data', array('scoid' => $oldscoid));
                    ...
                    if (!empty($id)) {
                        ...
                        if (isset($element->web_launch)) {
                            ...
                            $dataid = $DB->insert_record('svasu_scoes_data', $scodata);
                        }
    ...
    if (!empty($oldscoes)) {
        foreach ($oldscoes as $oldsco) {
            $DB->delete_records('svasu_scoes', array('id' => $oldsco->id));
            $DB->delete_records('svasu_scoes_track', array('scoid' => $oldsco->id));
        }
    }
...
/**
 * Given a svasuid creates an AICC Session record to allow HACP
 *
 * @param int $svasuid - id from svasu table
 * @return string hacpsession
 */
function svasu_aicc_get_hacp_session($svasuid) {
    ...
    $hacpsession = $SESSION->svasu;
    $hacpsession->svasuid = $svasuid;
 ...
 * @return mixed - false if invalid, otherwise returns record from svasu_aicc_session table.
 */
function svasu_aicc_confirm_hacp_session($hacpsession) {
    global $DB;
    $cfgsvasu = get_config('scorm');
    if (empty($cfgsvasu->allowaicchacp)) {
        return false;
    }
    $time = time() - ($cfgsvasu->aicchacptimeout * 60);
    $sql = "hacpsession = ? AND timemodified > ?";
    $hacpsession = $DB->get_record_select('svasu_aicc_session', $sql, array($hacpsession, $time));
    if (!empty($hacpsession)) { // Update timemodified as this is still an active session - resets the timeout.
        ...
        $DB->update_record('svasu_aicc_session', $hacpsession);
    }
    return $hacpsession;
}
 ...
 */
function svasu_aicc_generate_simple_sco($svasu) {
    global $DB;
    // Find the oldest one.
    $scos = $DB->get_records('svasu_scoes', array('svasu' => $svasu->id), 'id');
    if (!empty($scos)) {
    ...
    $sco->svasu = $svasu->id;
    $sco->organization = '';
    $sco->title = $svasu->name;
    $sco->parent = '/';
    // Add the HACP signal to the activity launcher.
    if (preg_match('/\?/', $svasu->reference)) {
        $sco->launch = $svasu->reference.'&CMI=HACP';
    } else {
        $sco->launch = $svasu->reference.'?CMI=HACP';
    }
    $sco->svasutype = 'sco';
...
function get_svasu_default (&$userdata, $svasu, $scoid, $attempt, $mode) {
    ...
    $def['cmi.student_data.attempt_number'] = svasu_isset($userdata, 'cmi.student_data.attempt_number');
    $def['cmi.student_data.mastery_score'] = svasu_isset($userdata, 'mastery_score');
    $def['cmi.student_data.max_time_allowed'] = svasu_isset($userdata, 'max_time_allowed');
    $def['cmi.student_data.time_limit_action'] = svasu_isset($userdata, 'time_limit_action');
    $def['cmi.student_data.tries_during_lesson'] = svasu_isset($userdata, 'cmi.student_data.tries_during_lesson');
