...
/**
 * Define all the backup steps that will be used by the backup_svasu_activity_task
 */

/**
 * Define the complete svasu structure for backup, with file and id annotations
 */
class backup_svasu_activity_structure_step extends backup_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $svasu = new backup_nested_element('svasu', array('id'), array(
            'name', 'svasutype', 'reference', 'intro',
        ...
        $sco = new backup_nested_element('sco', array('id'), array(
            ...
            'launch', 'svasutype', 'title', 'sortorder'));
        ...
        // Define sources
        $svasu->set_source_table('svasu', array('id' => backup::VAR_ACTIVITYID));

        // Order is important for several SVASU calls (especially svasu_scoes) in the following calls to set_source_table
        $sco->set_source_table('svasu_scoes', array('svasu' => backup::VAR_PARENTID), 'sortorder, id');
        $scodata->set_source_table('svasu_scoes_data', array('scoid' => backup::VAR_PARENTID), 'id ASC');
        $seqrulecond->set_source_table('svasu_seq_ruleconds', array('scoid' => backup::VAR_PARENTID), 'id ASC');
        $seqrulecondsdata->set_source_table('svasu_seq_rulecond', array('ruleconditionsid' => backup::VAR_PARENTID), 'id ASC');
        $seqrolluprule->set_source_table('svasu_seq_rolluprule', array('scoid' => backup::VAR_PARENTID), 'id ASC');
        $seqrolluprulecond->set_source_table('svasu_seq_rolluprulecond', array('rollupruleid' => backup::VAR_PARENTID), 'id ASC');
        $seqobjective->set_source_table('svasu_seq_objective', array('scoid' => backup::VAR_PARENTID), 'id ASC');
        $seqmapinfo->set_source_table('svasu_seq_mapinfo', array('objectiveid' => backup::VAR_PARENTID), 'id ASC');

        // All the rest of elements only happen if we are including user info
        if ($userinfo) {
            $scotrack->set_source_table('svasu_scoes_track', array('scoid' => backup::VAR_PARENTID), 'id ASC');
        }
        ...
        $svasu->annotate_files('mod_svasu', 'intro', null); // This file area hasn't itemid
        $svasu->annotate_files('mod_svasu', 'content', null); // This file area hasn't itemid
        $svasu->annotate_files('mod_svasu', 'package', null); // This file area hasn't itemid

        // Return the root element (svasu), wrapped into standard activity structure
        return $this->prepare_activity_structure($svasu);
    }
}
