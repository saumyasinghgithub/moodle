...
// Used need to debug cmi content (if you uncomment this, you must comment the definition inside SVASUapi1_3)
//var cmi = new Object();

//
// SVASU 1.3 API Implementation
//
function SVASUapi1_3(def, cmiobj, cmiint, cmicommentsuser, cmicommentslms, svasudebugging, svasuauto, svasuid, cfgwwwroot, sesskey, scoid, attempt, viewmode, cmid, currentorg, autocommit) {

    var prerequrl = cfgwwwroot + "/mod/svasu/prereqs.php?a=" + svasuid + "&scoid=" + scoid + "&attempt=" + attempt + "&mode=" + viewmode + "&currentorg=" + currentorg + "&sesskey=" + sesskey;
    var datamodelurl = cfgwwwroot + "/mod/svasu/datamodel.php";
    var datamodelurlparams = "id=" + cmid + "&a=" + svasuid + "&sesskey=" + sesskey + "&attempt=" + attempt + "&scoid=" + scoid;
    ...
    function initdatamodel(scoid){
        ...
        prerequrl = cfgwwwroot + "/mod/svasu/prereqs.php?a=" + svasuid + "&scoid=" + scoid + "&attempt=" + attempt + "&mode=" + viewmode + "&currentorg=" + currentorg + "&sesskey=" + sesskey;
        datamodelurlparams = "id=" + cmid + "&a=" + svasuid + "&sesskey=" + sesskey + "&attempt=" + attempt + "&scoid=" + scoid;
    ...
    function Initialize (param) {
        scoid = svasu_current_node ? svasu_current_node.scoid : scoid;
        initdatamodel(scoid);
                ...
                if ('true' == result) {
                    ...
                    if (adl.nav.request != '_none_') {
                        switch (adl.nav.request) {
                            case 'continue':
                                setTimeout('mod_svasu_launch_next_sco();',500);
                            break;
                            case 'previous':
                                setTimeout('mod_svasu_launch_prev_sco();',500);
                            break;
                            case 'choice':
                            break;
                            case 'exit':
                    ...
                    // trigger TOC update
                    var callback = M.mod_svasu.connectPrereqCallback;
                ...
                if (typeof datamodel[scoid][elementmodel] != "undefined") {
                    if (datamodel[scoid][elementmodel].mod != 'r') {
                        ...
                        if ((matches != null) && ((matches.join('').length > 0) || (value.length == 0))) {
                            ...
                            //Store data
                            if (errorCode == "0") {
                                if (autocommit && !(SVASUapi1_3.timeout)) {
                                    SVASUapi1_3.timeout = Y.later(60000, API_1484_11, 'Commit', [""], false);
                                }
    ...
    function Commit (param) {
        if (SVASUapi1_3.timeout) {
            SVASUapi1_3.timeout.cancel();
            SVASUapi1_3.timeout = null;
        }
        errorCode = "0";
        if (param == "") {
            if ((Initialized) && (!Terminated)) {
...
M.svasu_api.init = function(Y, def, cmiobj, cmiint, cmicommentsuser, cmicommentslms, svasudebugging, svasuauto, svasuid, cfgwwwroot, sesskey, scoid, attempt, viewmode, cmid, currentorg, autocommit) {
    window.API_1484_11 = new SVASUapi1_3(def, cmiobj, cmiint, cmicommentsuser, cmicommentslms, svasudebugging, svasuauto, svasuid, cfgwwwroot, sesskey, scoid, attempt, viewmode, cmid, currentorg, autocommit);
}
