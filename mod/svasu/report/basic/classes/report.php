...
class report extends \mod_svasu\report {
    ...
    public function display($svasu, $cm, $course, $download) {
        ...
        // Detailed report.
        $mform = new \mod_svasu_report_settings($PAGE->url, compact('currentgroup'));
        if ($fromform = $mform->get_data()) {
            $detailedrep = $fromform->detailedrep;
            $pagesize = $fromform->pagesize;
            set_user_preference('svasu_report_detailed', $detailedrep);
            set_user_preference('svasu_report_pagesize', $pagesize);
        } else {
            $detailedrep = get_user_preferences('svasu_report_detailed', false);
        ...
        if (empty($currentgroup)) {
            ...
            if (!$groupstudents = get_users_by_capability($contextmodule, 'mod/svasu:savetrack', 'u.id', '', '', '',
                                                            $currentgroup, '', false)) {
        ...
        if ( !$nostudents ) {
            ...
            if ($download) {
                ...
                $filename = clean_filename("$shortname ".format_string($svasu->name, true));
            }
            ...
            if ($detailedrep && $scoes = $DB->get_records('svasu_scoes', array("svasu" => $svasu->id), 'sortorder, id')) {
