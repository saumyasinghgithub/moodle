...
/**
 * This file contains tests for svasu events.
 *
 * @package    mod_svasu
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_svasu_event_testcase extends advanced_testcase {
    ...
    /** @var stdClass store svasu object */
    protected $eventsvasu;
    ...
    protected function setUp() {
        ...
        $this->eventsvasu = $this->getDataGenerator()->create_module('svasu', $record);
        $this->eventcm = get_coursemodule_from_instance('svasu', $this->eventsvasu->id);
    }
    ...
    public function test_attempt_deleted_event() {
        ...
        $this->resetAfterTest();
        svasu_insert_track(2, $this->eventsvasu->id, 1, 4, 'cmi.core.score.raw', 10);
        $sink = $this->redirectEvents();
        svasu_delete_attempt(2, $this->eventsvasu, 4);
        ...
        $this->assertInstanceOf('\mod_svasu\event\attempt_deleted', $event);
        ...
        $expected = array($this->eventcourse->id, 'svasu', 'delete attempts', 'report.php?id=' . $this->eventcm->id,
                4, $this->eventcm->id);
        $this->assertEventLegacyLogData($expected, $events[0]);
        $this->assertEventContextNotUsed($event);

        // Test event validations.
        \mod_svasu\event\attempt_deleted::create(array(
            'contextid' => 5,
            'relateduserid' => 2
        ));
        $this->fail('event \\mod_svasu\\event\\attempt_deleted is not validating events properly');
    }
    ...
    public function test_course_module_viewed_event() {
        $this->resetAfterTest();
        $event = \mod_svasu\event\course_module_viewed::create(array(
            'objectid' => $this->eventsvasu->id,
        ...
        $expected = array($this->eventcourse->id, 'svasu', 'pre-view', 'view.php?id=' . $this->eventcm->id,
                $this->eventsvasu->id, $this->eventcm->id);
    ...
    public function test_course_module_instance_list_viewed_event() {
        $this->resetAfterTest();
        $event = \mod_svasu\event\course_module_instance_list_viewed::create(array(
        ...
        // Check that the legacy log data is valid.
        $expected = array($this->eventcourse->id, 'svasu', 'view all', 'index.php?id=' . $this->eventcourse->id, '');
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_interactions_viewed_event() {
        ...
        $event = \mod_svasu\event\interactions_viewed::create(array(
            ...
            'context' => context_module::instance($this->eventcm->id),
            'courseid' => $this->eventcourse->id,
            'other' => array('attemptid' => 2, 'instanceid' => $this->eventsvasu->id)
        ));
        ...
        // Check that the legacy log data is valid.
        $expected = array($this->eventcourse->id, 'svasu', 'userreportinteractions', 'report/userreportinteractions.php?id=' .
                $this->eventcm->id . '&user=5&attempt=' . 2, $this->eventsvasu->id, $this->eventcm->id);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_interactions_viewed_event_validations() {
        $this->resetAfterTest();
        try {
            \mod_svasu\event\interactions_viewed::create(array(
                'context' => context_module::instance($this->eventcm->id),
                'courseid' => $this->eventcourse->id,
                'other' => array('attemptid' => 2)
            ));
            $this->fail("Event validation should not allow \\mod_svasu\\event\\interactions_viewed to be triggered without
                    other['instanceid']");
        } catch (Exception $e) {
    ...
    public function test_report_viewed_event() {
        ...
        // Check that the legacy log data is valid.
        $expected = array($this->eventcourse->id, 'svasu', 'report', 'report.php?id=' . $this->eventcm->id . '&mode=basic',
                $this->eventsvasu->id, $this->eventcm->id);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_sco_launched_event() {
        ...
        // Check that the legacy log data is valid.
        $expected = array($this->eventcourse->id, 'svasu', 'launch', 'view.php?id=' . $this->eventcm->id,
                          'url_to_content_that_was_laoded.php', $this->eventcm->id);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);

        // Test validations.
        \mod_svasu\event\sco_launched::create(array(
             'objectid' => $this->eventsvasu->id,
             'context' => context_module::instance($this->eventcm->id),
             'courseid' => $this->eventcourse->id,
        ));
        $this->fail('Event \\mod_svasu\\event\\sco_launched is not validating "loadedcontent" properly');
    }
    ...
    public function test_tracks_viewed_event() {
        $this->resetAfterTest();
        $event = \mod_svasu\event\tracks_viewed::create(array(
            'relateduserid' => 5,
            'context' => context_module::instance($this->eventcm->id),
            'courseid' => $this->eventcourse->id,
            'other' => array('attemptid' => 2, 'instanceid' => $this->eventsvasu->id, 'scoid' => 3)
        ));
        ...
        // Check that the legacy log data is valid.
        $expected = array($this->eventcourse->id, 'svasu', 'userreporttracks', 'report/userreporttracks.php?id=' .
                $this->eventcm->id . '&user=5&attempt=' . 2 . '&scoid=3', $this->eventsvasu->id, $this->eventcm->id);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_tracks_viewed_event_validations() {
        $this->resetAfterTest();
        try {
            \mod_svasu\event\tracks_viewed::create(array(
                'context' => context_module::instance($this->eventcm->id),
                'courseid' => $this->eventcourse->id,
                'other' => array('attemptid' => 2, 'scoid' => 2)
            ));
            $this->fail("Event validation should not allow \\mod_svasu\\event\\tracks_viewed to be triggered without
                    other['instanceid']");
        } catch (Exception $e) {
            $this->assertInstanceOf('coding_exception', $e);
        }
    ...
    public function test_user_report_viewed_event() {
        ...
        // Check that the legacy log data is valid.
        $expected = array($this->eventcourse->id, 'svasu', 'userreport', 'report/userreport.php?id=' .
                $this->eventcm->id . '&user=5&attempt=' . 2, $this->eventsvasu->id, $this->eventcm->id);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_user_report_viewed_event_validations() {
        $this->resetAfterTest();
        try {
            \mod_svasu\event\user_report_viewed::create(array(
                'context' => context_module::instance($this->eventcm->id),
                'courseid' => $this->eventcourse->id,
                'other' => array('attemptid' => 2)
            ));
            $this->fail("Event validation should not allow \\mod_svasu\\event\\user_report_viewed to be triggered without
                    other['instanceid']");
        } catch (Exception $e) {
            $this->assertInstanceOf('coding_exception', $e);
        }
    ...
    public function test_scoreraw_submitted_event($cmielement, $cmivalue) {
        $this->resetAfterTest();
        $event = \mod_svasu\event\scoreraw_submitted::create(array(
    ...
    public function get_scoreraw_submitted_event_validations() {
        return array(
            'scoreraw_submitted => missing cmielement' => array(
                null, '50',
                "Event validation should not allow \\mod_svasu\\event\\scoreraw_submitted " .
    ...
    public function test_scoreraw_submitted_event_validations($cmielement, $cmivalue, $failmessage, $excmessage) {
        ...
        try {
            ...
            \mod_svasu\event\scoreraw_submitted::create($data);
    ...
    public function test_status_submitted_event($cmielement, $cmivalue) {
        $this->resetAfterTest();
        $event = \mod_svasu\event\status_submitted::create(array(
            'other' => array('attemptid' => '2', 'cmielement' => $cmielement, 'cmivalue' => $cmivalue),
            'objectid' => $this->eventsvasu->id,
            'context' => context_module::instance($this->eventcm->id),
            'relateduserid' => $this->eventuser->id
    ...
    public function get_status_submitted_event_validations() {
        return array(
            'status_submitted => missing cmielement' => array(
                null, 'passed',
                "Event validation should not allow \\mod_svasu\\event\\status_submitted " .
                    "to be triggered without other['cmielement']",
                'Coding error detected, it must be fixed by a programmer: ' .
                    "The 'cmielement' must be set in other."
            ),
    ...
    public function test_status_submitted_event_validations($cmielement, $cmivalue, $failmessage, $excmessage) {
        ...
        try {
            ...
            \mod_svasu\event\status_submitted::create($data);
            $this->fail($failmessage);
        } catch (Exception $e) {
            $this->assertInstanceOf('coding_exception', $e);
            $this->assertEquals($excmessage, $e->getMessage());
