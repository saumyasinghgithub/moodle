...
$url = new moodle_url('/mod/svasu/report/userreporttracks.php', array('id' => $id,
    'user' => $userid,
    'attempt' => $attempt,
    'scoid' => $scoid));
...
$selsco = $DB->get_record('svasu_scoes', array('id' => $scoid), '*', MUST_EXIST);
...
$event = \mod_svasu\event\tracks_viewed::create(array(
    ...
    'other' => array('attemptid' => $attempt, 'instanceid' => $svasu->id, 'scoid' => $scoid)
));
...
$PAGE->navbar->add("$strattempt $attempt - ".fullname($user),
    new moodle_url('/mod/svasu/report/userreport.php', array('id' => $id, 'user' => $userid, 'attempt' => $attempt)));
$PAGE->navbar->add($selsco->title . ' - '. get_string('details', 'svasu'));

if ($trackdata = svasu_get_tracks($selsco->id, $userid, $attempt)) {
    if ($trackdata->status == '') {
...
$exportfilename = $courseshortname . '-' . format_string($svasu->name, true) . '-' . get_string('details', 'svasu');

$table = new flexible_table('mod_svasu-userreporttracks');
...
if (!$table->is_downloading($download, $exportfilename)) {
    ...
    format_string($selsco->title). ' - '. get_string('details', 'svasu'), 3);
}
$table->define_baseurl($PAGE->url);
$table->define_columns(array('element', 'value'));
$table->define_headers(array(get_string('element', 'svasu'), get_string('value', 'svasu')));
$table->set_attribute('class', 'generaltable generalbox boxaligncenter svasutrackreport');
...
foreach ($trackdata as $element => $value) {
    if (substr($element, 0, 3) == 'cmi') {
        ...
        if (stristr($element, '.id') !== false) {
            ...
            $string = "trackresult";
        } else if (stristr($element, '.student_response') !== false or // SVASU 1.2 value.
            stristr($element, '.learner_response') !== false) { // SVASU 2004 value.
        ...
        if (empty($string) || $table->is_downloading()) {
            ...
            $row[] = $element.$OUTPUT->help_icon($string, 'svasu');
        }
        if (strpos($element, '_time') === false) {
            $row[] = s($value);
        } else {
            $row[] = s(svasu_format_duration($value));
        }
        $table->add_data($row);
    }
}
