...
namespace svasureport_interactions;

defined('MOODLE_INTERNAL') || die();

require_once($CFG->dirroot.'/mod/svasu/report/interactions/responsessettings_form.php');

class report extends \mod_svasu\report {
    /**
     * displays the full report
     * @param \stdClass $svasu full SVASU object
     * @param \stdClass $cm - full course_module object
     * @param \stdClass $course - full course object
     * @param string $download - type of download being requested
     */
    public function display($svasu, $cm, $course, $download) {
        global $CFG, $DB, $OUTPUT, $PAGE;

        $contextmodule = \context_module::instance($cm->id);
        $action = optional_param('action', '', PARAM_ALPHA);
        $attemptids = optional_param_array('attemptid', array(), PARAM_RAW);
        $attemptsmode = optional_param('attemptsmode', SVASU_REPORT_ATTEMPTS_ALL_STUDENTS, PARAM_INT);
        $PAGE->set_url(new \moodle_url($PAGE->url, array('attemptsmode' => $attemptsmode)));

        if ($action == 'delete' && has_capability('mod/svasu:deleteresponses', $contextmodule) && confirm_sesskey()) {
            if (svasu_delete_responses($attemptids, $svasu)) { // Delete responses.
        ...
        // Detailed report.
        $mform = new \mod_svasu_report_interactions_settings($PAGE->url, compact('currentgroup'));
        if ($fromform = $mform->get_data()) {
            ...
            set_user_preference('svasu_report_pagesize', $pagesize);
            set_user_preference('svasu_report_interactions_qtext', $includeqtext);
            set_user_preference('svasu_report_interactions_resp', $includeresp);
            set_user_preference('svasu_report_interactions_right', $includeright);
            set_user_preference('svasu_report_interactions_result', $includeresult);
        } else {
            $pagesize = get_user_preferences('svasu_report_pagesize', 0);
            $includeqtext = get_user_preferences('svasu_report_interactions_qtext', 0);
            $includeresp = get_user_preferences('svasu_report_interactions_resp', 1);
            $includeright = get_user_preferences('svasu_report_interactions_right', 0);
            $includeresult = get_user_preferences('svasu_report_interactions_result', 0);
        }
        if ($pagesize < 1) {
            $pagesize = SVASU_REPORT_DEFAULT_PAGE_SIZE;
        }
        ...
        $candelete = has_capability('mod/svasu:deleteresponses', $contextmodule)
                && ($attemptsmode != SVASU_REPORT_ATTEMPTS_STUDENTS_WITH_NO);
        // Select the students.
        $nostudents = false;

        if (empty($currentgroup)) {
            // All users who can attempt scoes.
            if (!$students = get_users_by_capability($contextmodule, 'mod/svasu:savetrack', 'u.id', '', '', '', '', '', false)) {
        ...
        if ( !$nostudents ) {
            ...
            if ($download) {
                $filename = clean_filename("$course->shortname ".format_string($svasu->name, true, $formattextoptions));
            }
            ...
            $columns[] = 'attempt';
            $headers[] = get_string('attempt', 'svasu');
            $columns[] = 'start';
            $headers[] = get_string('started', 'svasu');
            $columns[] = 'finish';
            $headers[] = get_string('last', 'svasu');
            $columns[] = 'score';
            $headers[] = get_string('score', 'svasu');
            $scoes = $DB->get_records('svasu_scoes', array("svasu" => $svasu->id), 'sortorder, id');
            ...
            $select .= 'st.svasuid AS svasuid, st.attempt AS attempt, ' .
                    \user_picture::fields('u', array('idnumber'), 'userid') .
                    get_extra_user_fields_sql($coursecontext, 'u', '', array('email', 'idnumber')) . ' ';

            // This part is the same for all cases - join users and svasu_scoes_track tables.
            $from = 'FROM {user} u ';
            $from .= 'LEFT JOIN {svasu_scoes_track} st ON st.userid = u.id AND st.svasuid = '.$svasu->id;
            switch ($attemptsmode) {
                case SVASU_REPORT_ATTEMPTS_STUDENTS_WITH:
                    // Show only students with attempts.
                    $where = ' WHERE u.id ' .$usql. ' AND st.userid IS NOT NULL';
                    break;
                case SVASU_REPORT_ATTEMPTS_STUDENTS_WITH_NO:
                    // Show only students without attempts.
                    $where = ' WHERE u.id ' .$usql. ' AND st.userid IS NULL';
                    break;
                case SVASU_REPORT_ATTEMPTS_ALL_STUDENTS:
            ...
            for ($id = 0; $id < $questioncount; $id++) {
                if ($displayoptions['qtext']) {
                    $columns[] = 'question' . $id;
                    $headers[] = get_string('questionx', 'svasureport_interactions', $id);
                }
                if ($displayoptions['resp']) {
                    $columns[] = 'response' . $id;
                    $headers[] = get_string('responsex', 'svasureport_interactions', $id);
                }
                if ($displayoptions['right']) {
                    $columns[] = 'right' . $id;
                    $headers[] = get_string('rightanswerx', 'svasureport_interactions', $id);
                }
                if ($displayoptions['result']) {
                    $columns[] = 'result' . $id;
                    $headers[] = get_string('resultx', 'svasureport_interactions', $id);
                }
            }

            if (!$download) {
                $table = new \flexible_table('mod-svasu-report');
                ...
                $sheettitle = get_string('report', 'svasu');
            ...
            if (!$download) {
                ...
                echo \html_writer::start_div('svasuattemptcounts');
                if ( $count->nbresults == $count->nbattempts ) {
                    echo get_string('reportcountattempts', 'svasu', $count);
                } else if ( $count->nbattempts > 0 ) {
                    echo get_string('reportcountallattempts', 'svasu', $count);
                } else {
            ...
            if (!$download) {
                ...
                echo \html_writer::start_div('', array('id' => 'svasutablecontainer'));
                if ($candelete) {
                    // Start form.
                    $strreallydel  = addslashes_js(get_string('deleteattemptcheck', 'svasu'));
            ...
            if ($attempts) {
                foreach ($attempts as $scouser) {
                    ...
                    if (!empty($scouser->attempt)) {
                        $timetracks = svasu_get_sco_runtime($svasu->id, false, $scouser->userid, $scouser->attempt);
                    } else {
                        $timetracks = '';
                    }
                    ...
                    if (empty($timetracks->start)) {
                        ...
                        if (!$download) {
                            $url = new \moodle_url('/mod/svasu/report/userreport.php',
                                                    array('id' => $cm->id,
                                                        'user' => $scouser->userid,
                        ...
                        $row[] = svasu_grade_user_attempt($svasu, $scouser->userid, $scouser->attempt);
                    }
                    // Print out all scores of attempt.
                    $emptyrow = $download ? '' : '&nbsp;';
                    foreach ($scoes as $sco) {
                        if ($sco->launch != '') {
                            if ($trackdata = svasu_get_tracks($sco->id, $scouser->userid, $scouser->attempt)) {
                                if ($trackdata->status == '') {
                                ...
                                if (!$download) {
                                    $url = new \moodle_url('/mod/svasu/report/userreporttracks.php', array('id' => $cm->id,
                                        'scoid' => $sco->id, 'user' => $scouser->userid, 'attempt' => $scouser->attempt));
                                    $row[] = $OUTPUT->pix_icon($trackdata->status, $strstatus, 'svasu') . '<br>' .
                                        \html_writer::link($url, $score, array('title' => get_string('details', 'svasu')));
                                } else {
                                    $row[] = $score;
                                }
                                // Interaction data.
                                ...
                                $strstatus = get_string('notattempted', 'svasu');
                                if (!$download) {
                                    $row[] = $OUTPUT->pix_icon('notattempted', $strstatus, 'svasu') . '<br>' . $strstatus;
                                } else {
                                    $row[] = $strstatus;
                                }
            ...
            echo $OUTPUT->notification(get_string('noactivity', 'svasu'));
        }
    }// Function ends.
}
