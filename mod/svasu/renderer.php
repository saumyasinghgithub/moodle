...
/**
 * Defines the renderer for the svasu module.
 *
 * @package mod_svasu
...
/**
 * The renderer for the svasu module.
 *
 * @copyright 2013 Dan Marsden
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_svasu_renderer extends plugin_renderer_base {
    public function view_user_heading($user, $course, $baseurl, $attempt, $attemptids) {
        ...
        $pb = new mod_svasu_attempt_bar($attemptids, $attempt, $baseurl, 'attempt');
    ...
    /**
     * svasu attempt bar renderer
     *
     * @param mod_svasu_attempt_bar $attemptbar
     * @return string
     */
    protected function render_mod_svasu_attempt_bar(mod_svasu_attempt_bar $attemptbar) {
        $output = '';
        $attemptbar = clone($attemptbar);
        $attemptbar->prepare($this, $this->page, $this->target);

        if (count($attemptbar->attemptids) > 1) {
            $output .= get_string('attempt', 'svasu') . ':';
...
/**
 * Component representing a SVASU attempts bar.
 *
 * @copyright 2013 Dan Marsden
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @package mod_svasu
 */
class mod_svasu_attempt_bar implements renderable {
    ...
    /**
     * Constructor mod_svasu_attempt_bar with only the required params.
     *
    ...
    /**
     * Prepares the svasu attempt bar for output.
     *
     * This method validates the arguments set up for the svasu attempt bar and then
    ...
    public function prepare(renderer_base $output, moodle_page $page, $target) {
        if (empty($this->attemptids)) {
            throw new coding_exception('mod_svasu_attempt_bar requires a attemptids value.');
        }
        if (!isset($this->attempt) || is_null($this->attempt)) {
            throw new coding_exception('mod_svasu_attempt_bar requires a attempt value.');
        }
        if (empty($this->baseurl)) {
            throw new coding_exception('mod_svasu_attempt_bar requires a baseurl value.');
        }
