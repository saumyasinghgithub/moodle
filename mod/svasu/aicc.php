...
$url = new moodle_url('/mod/svasu/aicc.php', array('command' => $command, 'session_id' => $sessionid));
if ($aiccdata !== 0) {
...
if (empty($cfgsvasu->allowaicchacp)) {
    ...
    $aiccuser = $USER;
    $svasusession = $SESSION->svasu;
} else {
    $svasusession = svasu_aicc_confirm_hacp_session($sessionid);
    if (empty($svasusession)) {
        print_error('invalidhacpsession', 'svasu');
    }
    $aiccuser = $DB->get_record('user', array('id' => $svasusession->userid), 'id,username,lastname,firstname', MUST_EXIST);
}

if (!empty($command)) {
    $command = strtolower($command);

    if (isset($svasusession->scoid)) {
        $scoid = $svasusession->scoid;
    } else {
        print_error('cannotcallscript');
    }
    $mode = 'normal';
    if (isset($svasusession->svasumode)) {
        $mode = $svasusession->svasumode;
    }
    $status = 'Not Initialized';
    if (isset($svasusession->svasustatus)) {
        $status = $svasusession->svasustatus;
    }
    if (isset($svasusession->attempt)) {
        $attempt = $svasusession->attempt;
    } else {
        $attempt = 1;
    }

    if ($sco = svasu_get_sco($scoid, SCO_ONLY)) {
        if (!$svasu = $DB->get_record('svasu', array('id' => $sco->svasu))) {
            print_error('cannotcallscript');
        }
    } else {
        print_error('cannotcallscript');
    ...
    $aiccrequest = "MOODLE scoid: $scoid"
                 ...
                 . "\r\nAICC aiccdata:\r\n$aiccdata";
    svasu_debug_log_write("aicc", "HACP Request:\r\n$aiccrequest", $scoid);
    ob_start();
        ...
        switch ($command) {
            case 'getparam':
                if ($status == 'Not Initialized') {
                    $svasusession->svasustatus = 'Running';
                    $status = 'Running';
                }
                if ($status != 'Running') {
                    echo "error=101\r\nerror_text=Terminated\r\n";
                } else {
                    if ($usertrack = svasu_get_tracks($scoid, $aiccuser->id, $attempt)) {
                        $userdata = $usertrack;
                    } else {
                        $userdata->status = '';
                        $userdata->score_raw = '';
                    ...
                    if ($sco = svasu_get_sco($scoid)) {
                        ...
                        if (isset($userdata->{'cmi.core.lesson_status'})) {
                            ...
                            $svasusession->svasu_lessonstatus = $userdata->{'cmi.core.lesson_status'};
                        } else {
                            echo 'Lesson_Status=not attempted'.$userdata->entry."\r\n";
                            $svasusession->svasu_lessonstatus = 'not attempted';
                        }
            ...
            case 'putparam':
                if ($status == 'Running') {
                    if (! $cm = get_coursemodule_from_instance("svasu", $svasu->id, $svasu->course)) {
                        echo "error=1\r\nerror_text=Unknown\r\n"; // No one must see this error message if not hacked.
                    }
                    $savetrack = has_capability('mod/svasu:savetrack', context_module::instance($cm->id), $aiccuser->id);
                    if (!empty($aiccdata) && $savetrack) {
                        $initlessonstatus = 'not attempted';
                        $lessonstatus = 'not attempted';
                        if (isset($svasusession->svasu_lessonstatus)) {
                            $initlessonstatus = $svasusession->svasu_lessonstatus;
                        }
                        $score = '';
                        ...
                        foreach ($datarows as $datarow) {
                            if (($equal = strpos($datarow, '=')) !== false) {
                                ...
                                if (isset($datamodel[$element])) {
                                    ...
                                    switch ($element) {
                                        case 'cmi.core.lesson_location':
                                            $id = svasu_insert_track($aiccuser->id, $svasu->id, $sco->id,
                                        ...
                                        case 'cmi.core.session_time':
                                             $svasusession->sessiontime = $value;
                                        break;
                                    }
                                }
                            } else {
                                if (isset($datamodel[strtolower(trim($datarow))])) {
                                    ...
                                    $id = svasu_insert_track($aiccuser->id, $svasu->id, $sco->id, $attempt, $element, $value);
                                }
                            }
                        }
            ...
            case 'exitau':
                if ($status == 'Running') {
                    if (isset($svasusession->sessiontime) && ($svasusession->sessiontime != '')) {
                        if ($track = $DB->get_record('svasu_scoes_track', array("userid" => $aiccuser->id,
                                                                                "svasuid" => $svasu->id,
                                                                                "scoid" => $sco->id,
                                                                                "attempt" => $attempt,
                                                                                "element" => 'cmi.core.total_time'))) {
                            // Add session_time to total_time.
                            $value = svasu_add_time($track->value, $svasusession->sessiontime);
                            $track->value = $value;
                            ...
                            $track->svasuid = $svasu->id;
                            $track->scoid = $sco->id;
                            $track->element = 'cmi.core.total_time';
                            $track->value = $svasusession->sessiontime;
                            $track->attempt = $attempt;
                            $track->timemodified = time();
                            $id = $DB->insert_record('svasu_scoes_track', $track);
                        }
                        svasu_update_grades($svasu, $aiccuser->id);
                    }
                    $svasusession->svasustatus = 'Terminated';
                    $svasusession->session_time = '';
                    echo "error=0\r\nerror_text=Successful\r\n";
                } else if ($status == 'Terminated') {
                    echo "error=1\r\nerror_text=Terminated\r\n";
                } else {
...
if (empty($cfgsvasu->allowaicchacp)) {
    $SESSION->svasu = $svasusession;
} else {
    $svasusession->timemodified = time();
    $DB->update_record('svasu_aicc_session', $svasusession);
}

$aiccresponse = ob_get_contents();
svasu_debug_log_write("aicc", "HACP Response:\r\n$aiccresponse", $scoid);
ob_end_flush();
